# UrsuletTrandafirEternAd2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a new 15s Meta ad template (`UrsuletTrandafirEternAd2`) using existing ursulet-trandafir-etern assets, with pill-style subtitles, no heavy gradient, product-first hook, and 2–2.5s pacing.

**Architecture:** Single file `src/UrsuletTrandafirEternAd2.tsx` with local `PillSubtitleBlock` component, `Ad2Videos`, `Ad2Overlay`, and 4 format exports. Registered in `Root.tsx`. No new shared components — all new logic stays local to this file.

**Tech Stack:** Remotion 4.0 · React 19 · TypeScript · `@remotion/media` (Video) · `spring`, `interpolate` from remotion

---

## Clip durations (verified with ffprobe)

| Clip | Duration | Notes |
|------|----------|-------|
| `bear-closeup-4k.mp4` | 11.32s | 120fps 4K → playbackRate=0.25 |
| `gift-handover.mp4` | 10.87s | 30fps |
| `montova-gift.mp4` | 11.13s | 30fps |
| `cu-lantisor.mp4` | 5.13s | 30fps — max 5s without trimBefore |

CTA background: `gift-handover.mp4 trimBefore=5` → 5.87s available, covers 5s CTA ✅
...
|------|--------|---------------|
| `src/UrsuletTrandafirEternAd2.tsx` | Create | All components + 4 exports |
| `src/Root.tsx` | Modify | Register 4 new Compositions |

---

## Task 1: Create `src/UrsuletTrandafirEternAd2.tsx` — constants + PillSubtitleBlock

**Files:**
- Create: `src/UrsuletTrandafirEternAd2.tsx`

- [ ] **Step 1: Create the file with imports, constants, and `PillSubtitleBlock`**

Create `/Users/Eusebiu1/Desktop/workspace/Personal/azora/azora-ads/src/UrsuletTrandafirEternAd2.tsx` with this content:
...
```bash
cd /Users/Eusebiu1/Desktop/workspace/Personal/azora/azora-ads && npx tsc --noEmit 2>&1 | head -20
```

Expected: no errors (file is incomplete yet — OK if it complains about missing exports, not type errors).

---

## Task 2: Add `Ad2Videos` component

**Files:**
- Modify: `src/UrsuletTrandafirEternAd2.tsx` (append after PillSubtitleBlock)

- [ ] **Step 1: Append `Ad2Videos` to the file**

Append after the `PillSubtitleBlock` definition:
...
## Task 3: Add `Ad2Overlay` component

**Files:**
- Modify: `src/UrsuletTrandafirEternAd2.tsx` (append after Ad2Videos)

- [ ] **Step 1: Append `Ad2Overlay` to the file**
...
## Task 4: Add 9x16 + 4x5 format exports

**Files:**
- Modify: `src/UrsuletTrandafirEternAd2.tsx` (append)

- [ ] **Step 1: Append 9x16 and 4x5 exports**
...
## Task 5: Add 1x1 + 16x9 blurred backdrop exports

**Files:**
- Modify: `src/UrsuletTrandafirEternAd2.tsx` (append)

- [ ] **Step 1: Append 1x1 export**

```tsx
// ==========================================================================
// 1x1 — square: blurred backdrop + centered sharp portrait column
// objectFit:cover ar tăia ~22% top/bottom — blurred backdrop evită crop agresiv
// ==========================================================================
...
- [ ] **Step 2: Append 16x9 export** (identic cu 1x1)
...
In `src/Root.tsx`, after the existing `UrsuletTrandafirEternAd` import block, add:
...
- [ ] **Step 2: Add 4 Composition entries**

In `RemotionRoot`, after the `UrsuletTrandafirEternAd-16x9` composition, add:
...
```bash
cd /Users/Eusebiu1/Desktop/workspace/Personal/azora/azora-ads && npx tsc --noEmit 2>&1
```
...
## Task 7: Preview + visual verification checklist

- [ ] **Step 1: Start Remotion Studio**

```bash
cd /Users/Eusebiu1/Desktop/workspace/Personal/azora/azora-ads && npm run dev
```

Open `http://localhost:3000` and select `UrsuletTrandafirEternAd2-4x5`.

- [ ] **Step 2: Visual checklist — 4x5**

Play the composition and verify:
- [ ] Frame 0: bear slow-mo visible + pill text "Nu-i mai lua un cadou banal." apare imediat
- [ ] Pill background: `rgba(0,0,0,0.65)` cu border-radius vizibil (nu gradient full-width)
- [ ] Animație pop-in pe text: scale cu overshoot (nu fade simplu)
- [ ] f69: cut la `gift-handover.mp4`, text nou "Florile se ofilesc."
- [ ] f148: text mare "Acesta nu." — impactant, se vede că e mai mare (fontSize 58)
- [ ] f196: text 2 linii pe `cu-lantisor.mp4`
- [ ] f225: `MidCTAHint` apare subtil
- [ ] f300: CTA overlay cu "Comandă acum — livrare în 24h"
- [ ] Niciun gradient greu la bottom (verifică că nu există bandă violet/wine în zona inferioară a imaginii)
- [ ] Watermark dispare exact la f300

- [ ] **Step 3: Visual checklist — 9x16**

Switch to `UrsuletTrandafirEternAd2-9x16` — trebuie să fie identic cu 4x5 (same component).

- [ ] **Step 4: Visual checklist — 1x1 și 16x9**

Switch la `UrsuletTrandafirEternAd2-1x1`:
- [ ] Blurred backdrop vizibil în zonele laterale
- [ ] Portrait column centrat
- [ ] Overlay și timing sincronizate cu backdrop

Switch la `UrsuletTrandafirEternAd2-16x9`:
- [ ] Blurred backdrop vizibil sus și jos
- [ ] Portrait column centrat

---

## Render final (opțional — după aprobare vizuală)

```bash
cd /Users/Eusebiu1/Desktop/workspace/Personal/azora/azora-ads

npx remotion render UrsuletTrandafirEternAd2-4x5  out/ursulet-etern-ad2-4x5.mp4
npx remotion render UrsuletTrandafirEternAd2-9x16 out/ursulet-etern-ad2-9x16.mp4
npx remotion render UrsuletTrandafirEternAd2-1x1  out/ursulet-etern-ad2-1x1.mp4
npx remotion render UrsuletTrandafirEternAd2-16x9 out/ursulet-etern-ad2-16x9.mp4
```
